...
pub fn parse_package_arg<I: AsRef<str>>(input: I) -> Result<PackageArg> {
    ...
    match all_consuming(package_arg::<VerboseError<&str>>)(input) {
        Ok((_, arg)) => Ok(arg),
        Err(err) => Err(PackageArgError::ParseError).with_context(|| Ctx::DS1018 {
            input: input.into(),
            msg: format!(
                "{}",
                match err {
...
/// ent-pkg := not('/')+ '/' not('/')+ '/' not('@/')+ [ '@' version=req ]
fn ent_pkg<'a, E>(input: &'a str) -> IResult<&'a str, PackageArg, E>
where
    E: ParseError<&'a str>,
{
    context(
        ...
        map(
            tuple((
                map_res(take_till1(|c| c == '/'), |host| {
                    let mut string = String::from("https://");
                    string.push_str(host);
                    Url::parse(&string).and_then(|x| {
                        x.host()
                            .ok_or(url::ParseError::EmptyHost)
                            .map(|x| x.to_owned())
                    })
                }),
                preceded(tag("/"), map_res(take_till1(|c| c == '/'), no_url_encode)),
                preceded(
                    tag("/"),
                    map_res(take_till1(|c| c == '/' || c == '@'), no_url_encode),
                ),
